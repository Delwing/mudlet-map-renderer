...
// Streams a Mudlet .dat into the compact skeleton the StreamingMapReader needs,
// then transfers the typed-array buffers zero-copy to the main thread.
...
self.onmessage = async (e: MessageEvent<{file: File}>) => {
    try {
        ...
        const header = streamRooms(bytes, (id, room) => {
            X.push(room.x);
            Y.push(-room.y); // renderer space (MapReader negates y)
            Z.push(room.z);
            AREA.push(room.area);
